...
│   │       │   ├── mod.rs        # run_preflight(web_browser_executable: Option<&str>) -> PreflightOutcome { report, flutter_sdk } — orchestration entry point; emits 10 components on Linux, 11 on Windows (+ VisualStudioCpp appended via check_windows in the tokio::join! fan-out — Windows-only extend, no-op elsewhere), 12 on macOS (+ XcodeTools, CocoaPods appended via check_ios — macOS-only extend, no-op elsewhere); accepts a web_browser_executable override threaded from settings.toolchain.web_browser_executable via RunToolchainPreflight (pre-compute-in-preflight pattern; build_steps stays pure-on-report); returns the resolved FlutterSdk alongside the report so callers never need a second find_flutter_sdk call; on Windows, refreshes the process PATH from the registry (expanded Machine + User `Path` values, joined with `;`) before probing, so a wizard re-check detects tools installed after fdemon launched (e.g. via winget) without restarting fdemon; `#[cfg(windows)]`-only, no-op on Linux/macOS; re-exports Phase 2/3/4/5 install API + resolve_android_sdk_root_path
│   │       │   ├── types.rs      # Phase 1: ToolchainReport, ComponentCheck, ComponentKind (FlutterSdk, Git, Jdk, AndroidCmdlineTools, AndroidPlatformTools, AndroidPlatform, AndroidBuildTools, AndroidLicenses, Prerequisites, WebBrowser), etc. Phase 2: InstallMethod, HostArch, FlutterRelease, FlutterInstallTarget, DownloadProgress, FlutterInstallOutcome. Phase 3 (toolchain-bootstrap): AndroidInstallTarget, AndroidInstallOutcome, cmdline_tools_url, sdkmanager_packages, DEFAULT_CMDLINE_TOOLS_BUILD. Phase 3 (toolchain-platforms-submenu): added ComponentKind::WebBrowser for the web-browser probe. Phase 4: added ComponentKind::XcodeTools (Display "Xcode") + ComponentKind::CocoaPods (Display "CocoaPods") — macOS-only full-Xcode + CocoaPods detection, distinct from the CLT signal in Prerequisites. Phase 5: added ComponentKind::VisualStudioCpp (Display "Visual Studio (C++ workload)") — Windows-only; never emitted on Linux or macOS; 13 variants total
│   │       │   ├── checks/       # Per-component probes (mod.rs, android.rs, prerequisites.rs, web.rs, ios.rs, windows.rs) — no install or network code; android.rs owns resolve_android_sdk_root_path (shared SDK-root resolver) and android_sdk_root() (is_dir()-filtered wrapper); web.rs implements check_web(platform, browser_override): probe order is browser_override → CHROME_EXECUTABLE env → per-OS defaults (Linux: which google-chrome/chromium; macOS: /Applications/Google Chrome.app/…; Windows: Program Files/LocalAppData chrome.exe + msedge fallback); returns Ok with path when found, Missing when absent, Unknown on HostPlatform::Unknown; ios.rs (Phase 4, hardened Task 01) implements check_ios(platform) -> Vec<ComponentCheck>: one probe pass produces XcodeTools + CocoaPods checks; empty Vec on Linux/Windows, two Unknown checks on HostPlatform::Unknown, two Ok/Missing checks on macOS. XcodeTools probe runs a five-gate sequence — (1) xcode-select -p (path must resolve to a full Xcode.app bundle, not CLT), (2) xcodebuild -version (must succeed and return a parseable version), (3) xcodebuild -license check (exit 0 = accepted), (4) xcodebuild -checkFirstLaunchStatus (exit 0 = first-launch complete), (5) xcrun simctl list devices booted (exit 0 = simctl reachable; booted filter avoids known simctl list hang); XcodeTools = Ok iff all five gates pass — any failing gate yields Missing with a detail naming the specific gate and its remediation; gates 3–5 run concurrently with no short-circuit; a present-but-misconfigured Xcode always reports Missing (capped to non-blocking Partial at the leaf by build_steps); all spawned processes use kill_on_drop(true) and stdin(Stdio::null()) (read-only, non-interactive). CocoaPods probe runs pod --version; Ok with version or Missing on absence. windows.rs (Phase 5) implements check_windows(platform) -> Vec<ComponentCheck>: empty Vec on Linux/macOS, one Unknown check on HostPlatform::Unknown, one VisualStudioCpp check on Windows. Windows probe resolves vswhere.exe at %ProgramFiles(x86)%\Microsoft Visual Studio\Installer\vswhere.exe first, then via which::which("vswhere") as fallback; runs a two-gate query: gate 1 (-products * -latest -format json -utf8 — any VS instance, includes Build Tools SKUs), gate 2 (same plus -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 Microsoft.VisualStudio.Component.VC.CMake.Project — AND semantics, both required); classify_vswhere_gates is a pure function: gate-2 hit → Ok (displayName + version); gate-1-hit/gate-2-miss → Missing with detail starting with VS_FOUND_PREFIX ("Visual Studio found") — consumed by windows_guided_commands in fdemon-app to select modify-vs-install guidance; both-miss → Missing "Visual Studio not found"; all spawned processes use kill_on_drop(true), stdin(Stdio::null()), and PROBE_TIMEOUT
...
│   │       │   ├── state.rs      # InstallWizardState, WizardStep, build_steps(report, expanded), rollup_step_statuses(); Phase 2 (Platforms submenu) adds WizardStep.indent: u8, InstallWizardState.platforms_expanded: bool, and the two-mode build_steps projection: collapsed (5 rows: Prerequisites → Platforms → FlutterSdk → PathConfig → Doctor) vs expanded (parent + host-gated leaves after Platforms). Platforms parent status rolls up leaf statuses via rollup_step_statuses (Pending treated as neutral). Phase 3 (toolchain-bootstrap) adds GuidedCommand population for PlatformAndroid (JDK gate). Phase 3 (toolchain-platforms-submenu) adds PlatformWeb: build_steps routes WebBrowser onto the PlatformWeb leaf with a Missing→Partial cap (non-blocking — a missing browser surfaces as Partial, the Platforms parent rolls up to at most Partial, and handback via flutter_now_live / close_wizard_and_dispatch_discovery is unaffected because it reads only FlutterSdk); guided commands emitted per-OS only when Partial (browser absent), never for Ok or Pending. Phase 4 adds selected_command_index, select_next_command()/select_prev_command(), prerequisites_guided_commands(). Phase 4 (toolchain-platforms-submenu) graduates PlatformIos + PlatformMacos: build_steps routes XcodeTools/CocoaPods into both a platform_ios_components and a platform_macos_components bucket (cloned — shared probe, two leaves), caps each leaf Missing→Partial (non-blocking; same model as PlatformWeb), populates live leaf bodies inside the HostPlatform::MacOs host-gate; xcode_guided_commands(report, status, include_ios_platform) emits guided commands only when Partial — iOS leaf includes xcodebuild -downloadPlatform iOS, macOS leaf omits it; all_components_ok() remains strict (non-blocking for handback). Bugfix (informational-reopen) adds origin: WizardOrigin, is_bootstrap(), all_components_ok(). Phase 5 graduates PlatformWindows: build_steps routes ComponentKind::VisualStudioCpp into the PlatformWindows leaf inside the HostPlatform::Windows host-gate, caps Missing→Partial (non-blocking; same model as PlatformWeb/iOS/macOS), populates guided_commands via windows_guided_commands(report, status, components); windows_guided_commands branches on the VS_FOUND_PREFIX cross-crate detail-prefix contract from checks/windows.rs::classify_vswhere_gates — when the VisualStudioCpp detail starts with "Visual Studio found", emits a modify-existing-install entry first; always emits a winget entry when report.winget_available is true; always emits a choco entry as the universal fallback; commands emitted only when Partial (determined-absent workload), never for Ok or Pending.
│   │       │   ├── types.rs      # WizardStepKind: Prerequisites, Platforms (non-executable parent), PlatformAndroid, PlatformIos, PlatformMacos, PlatformWeb, PlatformWindows (platform leaves), PathConfig, FlutterSdk, Doctor. is_platform_leaf() returns true for the five platform leaf variants. StepStatus, WizardPane, StepExecution, StepExecStatus (Phase 2); GuidedCommand (Phase 3). Phase 5 followup: StepExecStatus::Cancelled. WizardOrigin { Bootstrap, UserInvoked }. Note: all five platform leaves are now live detect+guided-only leaves — PlatformAndroid, PlatformWeb, PlatformIos, PlatformMacos (Phases 2–4), and PlatformWindows (Phase 5).
│   │       │   └── version_picker.rs # Phase 6 (version picker): VersionPickerState, PickerRow, PickerChannel, PickerFetch, group_releases(manifest, arch) — pure grouping/arch-filter function; InstallWizardState gains version_picker field
...
│   │       │   ├── navigation.rs # Navigation handlers (up/down, pane switch); Phase 2 (Platforms submenu) adds handle_toggle_expand — flips platforms_expanded, rebuilds step list via build_steps, keeps cursor on the parent row, clamps selected_index, and resets selected_command_index; Esc collapse tier collapses an expanded submenu (first Esc) before falling through to close (second Esc); Phase 4 adds handle_prev_command/handle_next_command for [ / ] guided-command index cycling; Phase 2 followup adds handle_expand (InstallWizardExpand — directional expand, no-op unless on the collapsed Platforms parent) and handle_collapse (InstallWizardCollapse — directional collapse, no-op when already collapsed; re-anchors cursor to the parent when collapsing from a leaf); a single private set_platforms_expanded helper is the sole source of truth for every expand/collapse transition (toggle, directional, Esc-collapse) — it rebuilds the projected step list, re-anchors the cursor to the Platforms parent when collapsing from a leaf row, clamps selected_index, and resets selected_command_index
│   │       │   ├── actions.rs    # WizardStep* message handlers + completion chain (Phase 2); Phase 3: PlatformAndroid step gated on JDK Ok, PathConfig step now also writes ANDROID_HOME, InstallWizardCopyCommand handler copies selected_guided_command() to clipboard; Phase 4: Prerequisites Enter arm split from Doctor — emits guided "Run the listed command(s), then press r to re-check." message instead of the "later phase" stub; PlatformIos + PlatformMacos split from the PlatformWindows placeholder arm — iOS/macOS become guided-only arms (folded with PlatformWeb; show the run-commands message only when guided_commands is non-empty, never begin_step); PlatformWindows kept the "Available in a later phase" placeholder in Phase 4. Phase 5: PlatformWindows is promoted to a live guided-only arm — folded with PlatformWeb/iOS/macOS; shows the run-commands message when guided_commands is non-empty, never calls begin_step; never blocks flutter_now_live handback. Bugfix (informational-reopen): close_wizard_and_dispatch_discovery is now origin-gated — only fires for Bootstrap opens (not UserInvoked) and additionally skipped when a session is already running; a UserInvoked wizard is a read-only informational view that returns to UiMode::Normal on close without triggering device discovery.
│   │       │   └── version_picker.rs # Phase 6 (version picker): handle_open_picker, handle_close_picker, handle_up/down/next_tab, handle_refetch, handle_manifest_fetched/failed, handle_confirm; key intercept pattern for picker-visible keys
...
│               │   ├── step_detail.rs # Right pane: per-step detail view + Enter action hints; Phase 3 (toolchain-bootstrap) adds guided-command block (label, command, optional note) with c-key copy hint; Phase 3 (toolchain-platforms-submenu) adds PlatformWeb rendering: caption ("Browser required for flutter run -d chrome") + guided-command block with c-copy; "coming soon" hint suppressed when the Web leaf has guided commands; PlatformWeb is guided-only (not executable); Phase 4 (toolchain-platforms-submenu) adds PlatformIos ("Xcode required for iOS development") and PlatformMacos ("Xcode required for macOS development") captions via step_caption(); both variants added to the render_action_hint() matches! allowlist so the "coming soon" hint is suppressed when guided commands are present; iOS/macOS are guided-only (is_executable() returns false); Phase 6: component rows and doctor lines wrap via Paragraph::Wrap { trim: false } + local wrapped_height helper (per-item height advance) instead of clipping into 1-row rects; guided-command windowing (command_block_height / guided_section_full_height / compute_guided_window) threads width and accounts for wrapped row counts
...
│               │   └── version_picker.rs # Phase 6 (version picker): VersionPickerOverlay widget — tabs (Stable/Beta/Master), scrollable row list with selection highlight + git-only badge, loading/error states, footer hints; rendered on top of the panel when version_picker.visible
...
| `toolchain/flutter_install.rs` | Managed Flutter SDK install: `install_flutter` (git-clone default with `--` option terminator and ref validation before invocation, archive-download fallback honoring the configured channel in the archive path), atomic temp-dir-then-rename via `TempDirGuard` RAII (cleanup survives `JoinHandle::abort()`), non-fatal `flutter precache`. Reclaims incomplete `final_dir` partials on retry; `reclaim_stale_flutter_tmps` removes any leftover temp dirs from previous (possibly different-PID) runs at install start under the advisory lockfile. Serializes concurrent installs via an advisory lockfile (`LockGuard`) created atomically under the install root. `git_install` accepts and honours the `CancellationToken` — a cancelled clone returns `Error::Cancelled` cooperatively (git child is killed via `kill_on_drop(true)`). Manifest fetches carry timeouts; `FVM_CACHE_PATH` is validated to be absolute before use. Helpers: `fetch_release_manifest`, `archive_download_url`, `resolve_install_dir`. Event type: `InstallEvent`. Phase 6 (version picker): `validate_ref` is the renamed, widened ref validator — allows `+` for old hotfix tags (e.g. `1.12.13+hotfix.5`), rejects empty refs, a leading `-` (argument-injection guard), and a leading `+` or `.`; `validate_channel` is kept as a backwards-compatible alias. Both `target.channel` and `target.version_tag` (when set) are validated at the top of `install_flutter`. `git_install` uses `target.version_tag.as_deref().unwrap_or(&target.channel)` as the `-b` ref — pinned installs are shallow clones detached at the tag. Private `resolve_version_release(manifest, version, arch)` resolves a release by exact version string with two-pass arch matching (exact `dart_sdk_arch` first, any-arch fallback); used by `archive_install` when `target.version_tag` is `Some` — a manifest miss is a hard error (never the stable fallback); the channel→stable fallback remains for the `version_tag: None` path only. |
...
| `install_wizard/` | `InstallWizardState`, `WizardStep`, `WizardStepKind`, `StepStatus`, `WizardPane`, `build_steps(report, expanded)` — state types for the two-pane toolchain diagnostics modal. **Phase 2 (Platforms submenu)**: `WizardStepKind` gains `Platforms` (non-executable parent row) and five platform leaf variants — `PlatformAndroid` (functional in Phase 2), `PlatformIos`, `PlatformMacos`, `PlatformWeb`, `PlatformWindows` (host-gated; Phases 3–5 graduate them to live detect+guided-only leaves in order); `is_platform_leaf()` identifies the five leaf variants. `WizardStep` gains `indent: u8` (0 = top-level/parent, 1 = leaf). `InstallWizardState` gains `platforms_expanded: bool` (defaults `false`). `build_steps(report, expanded)` projects the visible step list: collapsed → 5 rows (`Prerequisites`, `Platforms`, `FlutterSdk`, `PathConfig`, `Doctor`); expanded → parent + host-gated leaves inserted after `Platforms` (`PlatformAndroid` + `PlatformWeb` on all hosts; `PlatformIos` + `PlatformMacos` on macOS; `PlatformWindows` on Windows). Host gating reads `report.platform` only — no `cfg!` macros. `Platforms` parent status is rolled up from its leaf statuses via `rollup_step_statuses`, which treats `Pending` as neutral so placeholder leaves do not degrade the parent. Phase 2 also adds `StepExecution` and `StepExecStatus` (Idle/Running/Succeeded/Failed install state and log tail on `InstallWizardState.execution`) plus `installed_sdk_path`. Phase 5 followup adds `StepExecStatus::Cancelled`. `StepExecution::log_tail` is a bounded `VecDeque<String>`; O(1) front-eviction. ANSI codes stripped at render time. Phase 3 adds `GuidedCommand` (`label`, `command`, `note`); `WizardStep.guided_commands: Vec<GuidedCommand>` populated by `build_steps()`. Phase 4 adds `selected_command_index: usize`; `select_next_command()` / `select_prev_command()`; `prerequisites_guided_commands(report, components)` (pure function of `ToolchainReport`). Phase 5 adds `install_task: Option<InstallTaskHandle>`; `flutter_now_live() -> bool`; `handback_done: bool`; `windows_guided_commands(report, status, components)` — pure function that emits winget/choco/modify-VS guided commands for the `PlatformWindows` leaf, branching on the `VS_FOUND_PREFIX` cross-crate detail-prefix contract. Phase 6: `jdk_guided_command` dispatches by `report.linux_package_manager`; `prerequisites_guided_commands` Linux path filters to only-missing packages. Phase 7: `apply_report` resets `execution` to default. **Phase 6 (version picker)**: new `install_wizard/version_picker.rs` module — `VersionPickerState` (lazy manifest fetch, `PickerChannel` tabs Stable/Beta/Master, cursor/scroll, confirmed selection); `PickerRow` (version, channel, `release_date`, `arch`, `git_only`); `PickerFetch` lifecycle enum (NotFetched/Loading/Loaded/Failed); `group_releases(manifest, arch)` — pure grouping/arch-filter function (drops dev-channel entries, collapses macOS dual-arch duplicates, appends synthetic git-only master/main rows). `InstallWizardState` gains `pub version_picker: VersionPickerState`. Key methods: `open()` (returns whether a fetch is needed), `close()`, `begin_fetch()`, `apply_manifest(manifest, arch)`, `apply_fetch_error(msg)`, `confirm() -> Option<PickerRow>`, `reset()`. |
| `handler/install_wizard/` | Navigation and action handlers for `UiMode::InstallWizard`. `mod.rs` is a re-export shim; `navigation.rs` holds the up/down and pane-switch handlers. **Phase 2 (Platforms submenu)** extends `navigation.rs` with `handle_toggle_expand` (`InstallWizardToggleExpand` message, dispatched by Enter on the `Platforms` parent): flips `platforms_expanded`, rebuilds the step list via `build_steps(report, expanded)`, keeps the cursor on the parent row (index unchanged), clamps `selected_index` defensively, resets `selected_command_index` to 0. No-op when the selected step is not `Platforms`. Esc collapse tier in `handle_escape`: when `platforms_expanded` is true, the first Esc collapses the submenu (rebuilds collapsed list, clamps `selected_index`) and returns without closing; a second Esc falls through to the existing close path. Priority order: cancel > collapse > close. Phase 2 also adds `actions.rs` — handles `WizardStepStarted/Log/Progress/DownloadProgress/Completed/Failed/Phase` messages, chains `PersistSettings` + `InstallWizardAutoConfigurePath` on `WizardStepCompleted(FlutterSdk)` or `WizardStepCompleted(PlatformAndroid)`. Phase 3 extends `actions.rs`: `PlatformAndroid` step is executable but JDK-gated. Phase 4 extends `navigation.rs` with `handle_prev_command` / `handle_next_command`. Phase 4 extends `actions.rs`: `Prerequisites` Enter arm split from `Doctor`. Phase 5 extends `actions.rs`: `handle_cancel_step`, `handle_preflight_completed` with `flutter_now_live()` handback, Esc/Hide with live SDK dispatches `DiscoverDevices`. **Phase 2 followup (directional navigation)** extends `navigation.rs` with `handle_expand` (`InstallWizardExpand` — no-op unless on the collapsed `Platforms` parent; sets `platforms_expanded = true`) and `handle_collapse` (`InstallWizardCollapse` — no-op when already collapsed; sets `platforms_expanded = false` and re-anchors the cursor to the `Platforms` parent when collapsing from a leaf row). All three paths (toggle, directional expand, directional collapse, Esc-collapse) route through the single private `set_platforms_expanded` helper, which is the sole source of truth for the rebuild/re-anchor/clamp/reset sequence. **Phase 6 (version picker)**: new `handler/install_wizard/version_picker.rs` handler module — `handle_open_picker` (guards: step must be `FlutterSdk`, no running step; calls `version_picker.open()`, dispatches `FetchFlutterReleaseManifest` when fetch is needed), `handle_close_picker`, `handle_up/down/next_tab`, `handle_refetch`, `handle_manifest_fetched` (calls `apply_manifest(manifest, HostArch::detect())`; stale arrivals with picker closed are harmless), `handle_manifest_fetch_failed`, `handle_confirm` (on `PickerFetch::Failed` closes picker and dispatches a default-channel un-pinned install as an offline escape hatch; otherwise routes through `handle_run_selected_step` so `begin_step`/`run_seq`/token minting remain single-sourced). The `FlutterSdk` arm of `handle_run_selected_step` (`actions.rs`) gains a gate: when `version_picker.selected_release.is_none()` on the first Enter, it opens the picker rather than starting immediately; once a `selected_release` is confirmed, `FlutterStepParams.version_tag` carries the picked version (or `None` for the fallback), and `InstallMethod` is overridden to `GitClone` for `git_only` rows. |
...
| `install_wizard/` | Two-pane toolchain diagnostics panel: `mod.rs` (panel entry), `step_list.rs` (left pane: ordered step list; **Phase 2** renders `WizardStep.indent` — leaf rows are indented one level under the `Platforms` parent; the `Platforms` parent shows a `▸`/`▾` caret reflecting `platforms_expanded`; the step-list height is dynamic to accommodate variable row counts; the footer hint changes to show expand/collapse guidance when the `Platforms` parent is selected; Phase 5 adds a run-failed badge `✗` on failed steps), `step_detail.rs` (right pane: per-step detail with Enter action hints for runnable steps; Phase 3 adds guided-command block — label, command text, optional alternative note, and `c` key copy hint; Phase 5 adds `PlatformWindows` rendering: caption "Visual Studio C++ workload required for Windows desktop builds" via `step_caption()`; added to the `render_action_hint()` matches! allowlist so the "coming soon" hint is suppressed when guided commands are present; Windows is guided-only, `is_executable()` returns false; **Phase 6** adds `FlutterSdk` caption `"Enter chooses a version to install · v opens the version picker"` via `step_caption()`; action hint shows `"▶ Press Enter to install Flutter <version>"` when a version pick is confirmed, `"▶ Press Enter to install Flutter SDK"` otherwise), `doctor_view.rs` (embedded `flutter doctor -v` output view), `progress.rs` (Phase 2: `StepProgress` widget — live progress bar, byte counter, and scrolling log tail shown while a step is running; Phase 5 adds "Esc cancels" hint in the `StepProgress` footer while a step is Running), **Phase 6** new `version_picker.rs` — `VersionPickerOverlay` widget rendered on top of the panel (only when `state.version_picker.visible`); layout: tabs row (Stable/Beta/Master with active tab highlighted) / scrollable list rows (version · date · arch, "git-only" badge for synthetic rows, selection highlight, `corrected_scroll` viewport) / footer hints; loading/error states; the wizard footer appends `[v] versions` when the FlutterSdk step is selected and the picker is closed |
...
| `fdemon doctor` | Run a read-only toolchain preflight check (`src/doctor.rs`), print `[STATUS] kind — detail` lines (status column is fixed-width, right-aligned to 4 chars) plus captured `flutter doctor` output to stdout. Exit 0 when all components relevant to the current project are healthy; exit 1 when any required component is degraded. Android-specific components (Android SDK, build-tools, platforms, licenses, etc.) only gate the exit code when an Android SDK is actually present — a pure Flutter project without Android configured exits 0 even if Android components are missing. `WebBrowser` is never gating: a missing or absent browser is printed in the component listing but does not contribute to a failing exit code, so a browser-less host (CI container, headless server) with a healthy Flutter + Android toolchain exits 0 — consistent with the Install Wizard's non-blocking Web treatment (missing browser caps to `Partial`, never blocks wizard handback). Doctor-incompatible top-level flags (`--headless`, `--dap-stdio`, `--dap-port`, `--log-dir`, `--dap-config`) are rejected at startup with exit 2 instead of being silently ignored. Loads project settings and honours the configured `[flutter] sdk_path` — the same SDK resolution path used by the TUI engine. Note: because `doctor` is a named clap subcommand, a Flutter project in a directory literally named `doctor` cannot be launched via the bare token `fdemon doctor`; use `fdemon ./doctor` as a workaround. |
...
handler::update() (install_wizard/actions.rs)
    │
    │  [FlutterSdk step — Phase 6 version picker gate]
    │  if version_picker.selected_release.is_none():
    │      open picker (begin_fetch if needed)
    │      → UpdateAction::FetchFlutterReleaseManifest (if fetch needed)
    │      return — install deferred until picker confirms
    │
    │  [FlutterSdk step — pick confirmed or fallback]
    ...
    │  version_dir_name = version_tag ?? channel  — Phase 6
    ▼
...
handle_action() in actions/mod.rs
    ...
    │     via run_streaming (git-clone, -b <version_tag or channel>)   — Phase 6
    │     or download_to_file + extract_archive (archive)
...
WizardStepCompleted(FlutterSdk, sdk_path=Some(p)):
    ...
    4. Message::InstallWizardAutoConfigurePath { kind: FlutterSdk }   ← NEW (auto-PATH-config)
       ...
       (PathConfig completion or failure triggers InstallWizardRerunPreflight downstream;
        there is no direct/parallel preflight emission from the FlutterSdk or PlatformAndroid
...
WizardStepCompleted(PlatformAndroid):
    → Message::InstallWizardAutoConfigurePath { kind: PlatformAndroid }  ← NEW
       → handle_auto_configure_path():
           begin_step(PathConfig)   (clears install_task, bumps run_seq)
...
- `InstallMethod`, `HostArch`, `FlutterRelease`, `FlutterReleaseManifest`, `FlutterInstallTarget`, `DownloadProgress`, `FlutterInstallOutcome` — Phase 2 install types (`toolchain/types.rs`); Phase 6 adds `FlutterRelease.release_date: Option<String>` and `FlutterInstallTarget.version_tag: Option<String>`
...
- `install_wizard::InstallWizardState` (`platforms_expanded: bool`, `version_picker: VersionPickerState`), `install_wizard::WizardStep` (`indent: u8`), `install_wizard::WizardStepKind` (`Platforms` parent + `PlatformAndroid`/`PlatformIos`/`PlatformMacos`/`PlatformWeb`/`PlatformWindows` leaves + `is_platform_leaf()`), `install_wizard::StepStatus`, `install_wizard::WizardPane` — toolchain diagnostics wizard state types; `build_steps(report, expanded)` projects the visible step list (collapsed: 5 rows; expanded: parent + host-gated leaves)
- `install_wizard::VersionPickerState`, `install_wizard::PickerRow`, `install_wizard::PickerChannel`, `install_wizard::PickerFetch`, `install_wizard::group_releases` — Phase 6 version picker state and pure grouping/filter function (`install_wizard/version_picker.rs`)
