...
│   │       │   └── types.rs      # WizardStepKind: Prerequisites, Platforms (non-executable parent), PlatformAndroid, PlatformIos, PlatformMacos, PlatformWeb, PlatformWindows (platform leaves), PathConfig, FlutterSdk, Doctor. is_platform_leaf() returns true for the five platform leaf variants. StepStatus, WizardPane, StepExecution, StepExecStatus (Phase 2); GuidedCommand (Phase 3). Phase 5 followup: StepExecStatus::Cancelled. WizardOrigin { Bootstrap, UserInvoked }. Note: PlatformAndroid, PlatformWeb, PlatformIos, and PlatformMacos are now live detect+guided-only leaves; PlatformWindows remains the sole host-gated placeholder (StepStatus::Pending) arriving in Phase 5.
