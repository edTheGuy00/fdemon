...
- **VM WebSocket + DevTools polling lifecycle**: The VM Service WebSocket is established at session start and lives until session close. It is shared by DAP, the app-driver, one-shot inspector calls, and any MCP/headless consumer. Closing the DevTools page (Esc) does **not** tear down the socket — it only **pauses** the four polling loops (performance/memory, allocation, network, timeline) by sending `true` on their `watch` channels, so each loop body executes a true no-op `continue` with zero RPC traffic. The socket stays open and the pause channels are stored on `SessionHandle`; full task abort and socket teardown happen only on session close (`handler/session.rs`). A `devtools_service_monitoring` flag on `SessionHandle` (set by `StartDevToolsMonitoring` for MCP/headless consumers) suppresses the perf and network pause so those consumers keep receiving telemetry; alloc and timeline are always paused on DevTools close. See `handler/devtools/mod.rs:handle_exit_devtools_mode`, `actions/performance.rs`, and `actions/network.rs`.
