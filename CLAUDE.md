...
- **ADB QR pairing**: `fdemon-daemon/adb_pairing.rs` implements Android Studio's "Pair device with QR code" flow (mDNS browse via `mdns-sd` + `adb pair`/`adb connect`); surfaced as the New Session dialog's third "Pair QR" tab (`widgets/new_session_dialog/qr_pairing.rs`, rendered with `tui-qrcode`)
