...
|---|------|--------|------------|------------|-------|-----------|
| 1 | [01-split-extensions-submodules](tasks/01-split-extensions-submodules.md) | Not Started | - | 2-3h | `fdemon-daemon` | `vm_service/extensions/` |
| 2 | [02-refactor-client-ownership](tasks/02-refactor-client-ownership.md) | Not Started | 1 | 2-3h | `fdemon-daemon` | `vm_service/extensions/inspector.rs` |
| 3 | [03-error-handling-fixes](tasks/03-error-handling-fixes.md) | Not Started | 1 | 1-2h | `fdemon-daemon` | `vm_service/extensions/inspector.rs`, `layout.rs` |
| 4 | [04-client-rwlock-poison-safety](tasks/04-client-rwlock-poison-safety.md) | Not Started | - | 0.5-1h | `fdemon-daemon` | `vm_service/client.rs` |
| 5 | [05-minor-quality-fixes](tasks/05-minor-quality-fixes.md) | Not Started | 1 | 1-2h | `fdemon-daemon`, `fdemon-core` | Multiple |
...
- [ ] `ObjectGroupManager` does not own a `VmServiceClient`
- [ ] `WidgetInspector` uses a single client reference consistently (no dual-client pattern)
- [ ] `dispose_all` has no unused parameters
- [ ] `get_root_widget_tree` fallback only triggers on "extension not available" errors
- [ ] `create_group` succeeds even when old group dispose fails
- [ ] `extract_layout_tree` logs warnings when children fail to parse
- [ ] All `RwLock::unwrap()` replaced with poison-safe alternatives in `client.rs`
- [ ] Magic number 113 replaced with named constant
- [ ] `query_all_overlays` doc matches implementation (sequential, not concurrent)
- [ ] `cargo fmt --all` clean
