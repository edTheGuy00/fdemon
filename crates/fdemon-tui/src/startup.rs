...
use fdemon_app::session::SessionId;
...
// Stub module for dead_code functions (phase 4 cleanup)
// These functions are never called but need to compile
#[allow(dead_code)]
mod devices_stub {
    use fdemon_app::Device;
    ...
    pub async fn discover_devices() -> Result<DiscoveryResult> {
        unimplemented!("dead code - removed in phase 4")
    }

    pub fn find_device<'a>(_devices: &'a [Device], _device_id: &str) -> Option<&'a Device> {
        unimplemented!("dead code - removed in phase 4")
    }
}

// Use stub for dead code
#[allow(dead_code)]
use devices_stub as devices;
