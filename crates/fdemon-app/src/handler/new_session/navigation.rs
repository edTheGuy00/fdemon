...
pub fn handle_switch_tab(
    state: &mut AppState,
    tab: crate::new_session_dialog::TargetTab,
) -> UpdateResult {
    // Check if we need to trigger discovery BEFORE switch_tab modifies state
    let needs_bootable_discovery = tab == crate::new_session_dialog::TargetTab::Bootable
        && state
...
pub fn handle_field_activate(
    ...
    use crate::new_session_dialog::{DialogPane, FuzzyModalType, LaunchContextField};
