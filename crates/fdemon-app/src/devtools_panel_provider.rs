...
pub trait DevToolsPanelProvider: std::fmt::Debug + Send + 'static {
    ...
    ///
    /// Called only when this panel is the active DevTools panel. The host routes
    /// `Tab`/`Shift+Tab` (panel cycling), `Esc` (exit DevTools), and the
    /// built-in panel-switch letters (`i`/`p`/`m`/`n`) before the provider sees
    /// them, so those keys are never delivered here. Return [`Handled::Consumed`]
    /// if the key was used.
