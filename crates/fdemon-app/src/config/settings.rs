...
"#;
    ...
    #[test]
    fn test_init_fdemon_directory_config_references_fdemon_dev_docs() {
        let temp = tempdir().unwrap();
        init_fdemon_directory(temp.path()).unwrap();
        let content = std::fs::read_to_string(temp.path().join(".fdemon/config.toml")).unwrap();
        assert!(
            content.contains("https://fdemon.dev/docs/configuration"),
            "config.toml generated by init must point at fdemon.dev docs"
        );
        assert!(
            !content.contains("github.com/example"),
            "config.toml generated by init must not carry the placeholder URL"
        );
    }
