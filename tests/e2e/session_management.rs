...
fn test_session_removal() {
    ...
    let removed = manager.remove_session(id1);
...
fn test_removing_selected_session_selects_another() {
    ...
    // Remove selected
    manager.remove_session(ids[1]);

    // Should have selected another (not panic)
    assert!(manager.selected().is_some() || manager.is_empty());
}
...
fn test_number_key_selects_session() {
    ...
    let _result = update(&mut state, Message::SelectSessionByIndex(2));
...
fn test_session_with_same_device_allowed() {
    ...
    let id1 = manager.create_session(&device);
    let id2 = manager.create_session(&device);
